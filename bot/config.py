# START OF FILE: iPMxBT-main/bot/config.py
...
class Config(object):
    ...
    OWNER_ID = int(os.environ.get("OWNER_ID", "1413767412"))
    ...
    UPI_ID = os.environ.get("UPI_ID", "ksbaaghi@ybl")
    UPI_NAME = os.environ.get("UPI_NAME", "Kirana")
    PAYMENT_PAGE_URL = os.environ.get("PAYMENT_PAGE_URL", "https://cornpay.pages.dev")
...
class Script(object):
    ...
    # Naya, detailed user help message
    USER_HELP_MESSAGE = """
🙋‍♂️ **User Help Menu** 🙋‍♀️
...
Simply type the name of any movie or series in our private chat.
» Example: `Krrish`
...
**/status**
» Check your current premium subscription status and expiry date.

**/help** or **/userrights**
» Shows this help menu.
"""

    # Naya, detailed admin help message
    ADMIN_HELP_MESSAGE = """
👑 **Admin Command Panel** 👑
...
`/verify <amount>`
» Manually activates a user's subscription using their unique payment amount.
» Example: `/verify 20.57`

`/removesub <user_id>`
» Instantly removes a user's premium subscription.
» Example: `/removesub 123456789`

**Automatic Verification:**
» Simply copy-paste the payment confirmation SMS from your bank into my PM. I will automatically verify the user.

**User & Group Management:**
`/broadcast`
» Reply to any message and use this command to send it to all bot users.

`/info <group_id>`
» Shows details of a premium group and gives buttons to extend their subscription.
» Example: `/info -100123456789`

`/premium_groups`
» Shows a list of all active premium groups.

`/premium_reminder`
» Sends a subscription expiry reminder to all groups whose plan is ending soon.

**Utility Commands:**
`/add_caption <channel_id>`
» (Advanced) Adds file names as captions to all files in a channel that don't have one.

`/admincheck`
» Shows your admin status, debug info, and this command list.
"""
    # === END: YAHAN PAR HELP MESSAGES UPDATE KIYE GAYE HAIN ===
